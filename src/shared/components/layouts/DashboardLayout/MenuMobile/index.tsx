...
export const MenuMobileButton = ({ isOpen, onChange }: Props) => {
  return (
    <Box className="flex lg:hidden">
      ...
      <Box className="hover:text-primary z-10">
      ...
      >
        {[
          ...
          <Link key={item.label} href={item.href}>
            <Text className="flex items-center justify-center h-10 border-b-2 border-primaryHover bg-primary text-white hover:bg-primaryHover ">
              {item.label}
            </Text>
          </Link>
        ))}
